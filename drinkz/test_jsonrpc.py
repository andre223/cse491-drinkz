...
def test_rpc_convert_gallon_to_ml():        
    ...
    results = myApp.__call__(environ,my_start_response)
    text = "".join(results)    
    assert text.find("151416.472") != -1, text
...
def test_rpc_add_bottle_type():
    ...
    d = dict(method='add_bottle_type', params=["Smirnoff's","Rasberry Vodka","Vodka"] ,id=1)
    encoded = simplejson.dumps(d)
    ...
    assert text.find("Smirnoff's") != -1, text
    assert text.find("Rasberry") != -1, text
...
def test_rpc_add_to_inventory():
    ...
    d = dict(method='add_to_inventory', params=["Smirnoff's","Rasberry Vodka","400 ml"] ,id=1)
    encoded = simplejson.dumps(d)
...
def test_rpc_add_to_inventory2():
    initialize_db()
    myApp = app.SimpleApp()
